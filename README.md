...
**Web search, content extraction, and video understanding for Pi agent. Zero-config Exa search or bring your own API keys. Browser-cookie Gemini Web access is disabled in this fork.**
...
**Zero Config** — Works out of the box with Exa MCP (no API key needed). Add API keys for Exa, Perplexity, or Gemini API for more control. Browser-cookie access for Gemini Web is disabled in this fork.

**Video Understanding** — Point it at a YouTube video or local screen recording and ask questions about what's on screen. Full transcripts, visual descriptions, and frame extraction at exact timestamps.

**Smart Fallbacks** — Every capability has a fallback chain. Search tries Exa, then Perplexity, then Gemini API. YouTube tries Gemini API, then Perplexity. Blocked pages retry through Jina Reader and Gemini extraction. Something always works.
...
In `auto` mode (default), `web_search` tries Exa first (direct API if keyed, MCP if not), then Perplexity, then Gemini API. Browser-cookie Gemini Web access is disabled in this fork.
...
Fallback: Gemini API → Perplexity (text summary only). Handles all URL formats: `/watch?v=`, `youtu.be/`, `/shorts/`, `/live/`, `/embed/`, `/v/`.
...
Fallback: Gemini API (Files API upload). Browser-cookie Gemini Web access is disabled in this fork.
...
When Readability fails or returns only a cookie notice, the extension retries via Jina Reader (handles JS rendering server-side, no API key needed), then Gemini URL Context API. Handles SPAs, JS-heavy pages, and anti-bot protections transparently. Also parses Next.js RSC flight data when present.

## How It Works

```
web_search(query)
  → Exa (direct API with key, MCP without) → Perplexity → Gemini API

fetch_content(url)
  → Video file?  Gemini API (Files API)
  → GitHub URL?  Clone repo, return file contents + local path
  → YouTube URL? Gemini API → Perplexity
...
Reports that Gemini Web browser-cookie access is disabled in this fork.
...
- Chromium cookie extraction for Gemini Web is disabled in this fork. `allowBrowserCookies`, `PI_ALLOW_BROWSER_COOKIES`, and `FEYNMAN_ALLOW_BROWSER_COOKIES` are ignored.
...
| `gemini-search.ts` | Search routing across Exa, Perplexity, and Gemini API |
| `gemini-url-context.ts` | Gemini URL Context + Web extraction fallbacks |
| `gemini-web.ts` | Gemini Web client (cookie auth, StreamGenerate; gated off in this fork) |
| `gemini-web-config.ts` | Gemini Web profile config and disabled browser-cookie gate |
| `gemini-api.ts` | Gemini REST API client (generateContent) |
| `chrome-cookies.ts` | macOS/Linux Chromium-based cookie extraction (unused while browser-cookie access is disabled) |
| `youtube-extract.ts` | YouTube detection, extraction fallback, frame extraction |
